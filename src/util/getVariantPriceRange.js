...
export default function getVariantPriceRange(variantId, variants) {
  const visibleOptions = variants.filter(
    (option) =>
      option.ancestors.includes(variantId) &&
      option.isVisible &&
      !option.isDeleted
  );

  // all options include visible and hidden options. Deleted options are not considered
  const allOptions = variants.filter(
    (option) => option.ancestors.includes(variantId) && !option.isDeleted
  );
